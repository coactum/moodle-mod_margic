...
class mod_margic_renderer extends plugin_renderer_base {
     ...
     *
     * @param int $margic margic activity this description is for.
     * @param int $cm Course module this description is for.
     * @return $output Return a box with the description in it.
     */
    public function introduction($margic, $cm) {
        $output = '';

        if (trim($margic->intro)) {
            $output .= $this->box_start('generalbox boxaligncenter', 'intro');
            $output .= format_module_intro('margic', $margic, $cm->id);
            $output .= $this->box_end();
        }
        return $output;
    }
    ...
    public function toolbar($firstkey) {
        ...
        $url = new moodle_url('/mod/margic/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('i/export', get_string('csvexport', 'margic')), array(
            'class' => 'toolbutton'
        ));
        ...
        $url = new moodle_url('/mod/margic/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/reload', get_string('reload', 'margic')), array(
            'class' => 'toolbutton'
        ));
        ...
        $output .= html_writer::link($url, $this->pix_icon('i/edit', get_string('edittopoflist', 'margic')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/margic/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/left', get_string('sortfirstentry', 'margic')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/margic/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/down', get_string('lowestgradeentry', 'margic')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/margic/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/up', get_string('highestgradeentry', 'margic')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/margic/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/right', get_string('latestmodifiedentry', 'margic')), array(
            'class' => 'toolbutton'
        ));
    ...
    /**
     * Returns HTML for a margic inaccessible message.
     * Added 20161002
     *
     * @param string $message
     * @return <type>
     */
    public function margic_inaccessible($message) {
        global $CFG;
        $output = $this->output->box_start('generalbox boxaligncenter');
        $output .= $this->output->box_start('center');
        $output .= (get_string('notavailable', 'margic'));
        $output .= $message;
        $output .= $this->output->box('<a href="'
            .$CFG->wwwroot.'/course/view.php?id='
            .$this->page->course->id.'">'
            .get_string('returnto', 'margic', format_string($this->page->course->fullname, true))
            .'</a>', 'margicbutton standardbutton');
