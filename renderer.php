...
/**
 * This file contains a renderer for various parts of the annotateddiary module.
 *
...
/**
 * A custom renderer class that extends the plugin_renderer_base and is used by the annotateddiary module.
 *
 * @package mod_annotateddiary
 * @copyright 2019 onwards AL Rachels drachels@drachels.com
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_annotateddiary_renderer extends plugin_renderer_base {

    /**
     * Rendering annotateddiary files.
     *
     * @var int $annotateddiary
     */
    private $annotateddiary;
     ...
     *
     * @param int $annotateddiary annotateddiary activity this description is for.
     * @param int $cm Course module this description is for.
     * @return $output Return a box with the description in it.
     */
    public function introduction($annotateddiary, $cm) {
        $output = '';

        if (trim($annotateddiary->intro)) {
            $output .= $this->box_start('generalbox boxaligncenter', 'intro');
            $output .= format_module_intro('annotateddiary', $annotateddiary, $cm->id);
            $output .= $this->box_end();
        }
        return $output;
    }
    ...
    public function toolbar($firstkey) {
        ...
        $url = new moodle_url('/mod/annotateddiary/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('i/export', get_string('csvexport', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
        ...
        $url = new moodle_url('/mod/annotateddiary/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/reload', get_string('reload', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
        ...
        $output .= html_writer::link($url, $this->pix_icon('i/edit', get_string('edittopoflist', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/annotateddiary/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/left', get_string('sortfirstentry', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/annotateddiary/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/down', get_string('lowestgradeentry', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/annotateddiary/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/up', get_string('highestgradeentry', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
        ...
        $options['firstkey'] = $firstkey;
        $url = new moodle_url('/mod/annotateddiary/view.php', $options);
        $output .= html_writer::link($url, $this->pix_icon('t/right', get_string('latestmodifiedentry', 'annotateddiary')), array(
            'class' => 'toolbutton'
        ));
    ...
    /**
     * Returns HTML for a annotateddiary inaccessible message.
     * Added 20161002
     *
     * @param string $message
     * @return <type>
     */
    public function annotateddiary_inaccessible($message) {
        global $CFG;
        $output = $this->output->box_start('generalbox boxaligncenter');
        $output .= $this->output->box_start('center');
        $output .= (get_string('notavailable', 'annotateddiary'));
        $output .= $message;
        $output .= $this->output->box('<a href="'
            .$CFG->wwwroot.'/course/view.php?id='
            .$this->page->course->id.'">'
            .get_string('returnto', 'annotateddiary', format_string($this->page->course->fullname, true))
            .'</a>', 'annotateddiarybutton standardbutton');
     ...
     * @param object $grades
     */
    public function annotateddiary_print_feedback($course, $entry, $grades) {
        global $CFG, $DB;
